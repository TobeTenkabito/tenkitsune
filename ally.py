...
class Ally:
    def __init__(self, number, name, description, level, hp, mp, max_hp, max_mp, attack, defense, speed, crit, crit_damage,
                 resistance, penetration, skills=None, equipment=None):
        self.number = number
        self.name = name
    ...
    def __deepcopy__(self, memo):
        new_ally = type(self)(
            ...
            self.resistance,
            self.penetration,
            copy.deepcopy(self.skills, memo),  # 深拷贝技能列表
            copy.deepcopy(self.equipment, memo)  # 深拷贝装备列表
        )

        # 深拷贝其他属性
        new_ally.dizzy_rounds = self.dizzy_rounds
        new_ally.paralysis_rounds = self.paralysis_rounds
        new_ally.silence_rounds = self.silence_rounds
        new_ally.blind_rounds = self.blind_rounds
        new_ally.sustained_effects = copy.deepcopy(self.sustained_effects, memo)
        new_ally.buffs = copy.deepcopy(self.buffs, memo)
        new_ally.battle = self.battle
    ...
    def calculate_damage(self, target, base_value, skill_multiplier=1.0, is_skill=False):
        ...
        if 20 > level_difference >= 10:
            level_suppression = 1.1
        elif -20 < level_difference <= -10:
            level_suppression = 0.9
    ...
    def __str__(self):
        return (f"Ally({self.name}, Level: {self.level}, HP: {self.hp}/{self.max_hp}, "f"MP: {self.mp}/{self.max_mp}, ATK: {self.attack}, "
